...
export async function seedHistorialEstudiante(
  ...
  const ofertaLabels = [
    ...
    { key: 'oferta-circuitos', titulo: 'Circuitos Eléctricos - Análisis AC/DC' },
  ...
  console.log(`   → ${ESTUDIANTE_HU40_ID} tiene 4 COMPLETADAS + 2 NO_SHOW`);
  ...
  const reviews: Partial<ReviewEntity>[] = [
    // R1 — Álgebra Lineal con Juan Carlos Pérez (C1)
    {
      ...
      tutorId: TUTOR_001,
      rating: 5,
      comment: 'Muy buena sesión de álgebra lineal. Explica con mucha claridad.',
    },
  ...
  for (const r of reviews) {
    ...
    await reviewRepository.save(entity);
  }
